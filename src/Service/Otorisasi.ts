...
export class Otorisasi {
  ...
  private constructor(public creds: OtorisasiCredentialsMaping) {}
  ...
  public static async build(baseUrl: string, clientCredential: OtorisasiCredentials[]): Promise<Otorisasi> {
    if (!Otorisasi.client) {
      ...
      for (const credData of clientCredential) {
        ...
        const authResponse = await Otorisasi.getClientToken(clientId, clientSecret, baseUrl)
        creds[clientId] = {
      ...
      Otorisasi.client = new Otorisasi(creds)
    }
    return Otorisasi.client
  }


  private static async getClientToken(clientId: string, clientSecret: string, baseUrl: string): Promise<any> {
    try {
